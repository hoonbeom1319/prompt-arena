...
interface NewChallengeFormProps {
  initialConflict: { title: string; state: 'submission' | 'voting' } | null
}

// 챌린지 출제 폼 island. 활성 챌린지 충돌 여부는 서버에서 props로 받는다.
// 생성은 /api/admin/challenges 경유 — supabase 직접 호출 없음 (ARCHITECTURE §2).
export default function NewChallengeForm({ initialConflict }: NewChallengeFormProps) {
      ...
      {initialConflict && (
        <div role="alert" className="px-4 py-3.5 bg-[color-mix(in_oklab,var(--warning)_12%,white)] border border-[color-mix(in_oklab,var(--warning)_32%,white)] rounded-lg mb-5">
          ...
          <p className="text-sm text-text-secondary">
            &lsquo;{initialConflict.title}&rsquo;이 현재{' '}
            <strong>{initialConflict.state === 'submission' ? '제출' : '투표'} 중</strong>입니다.
