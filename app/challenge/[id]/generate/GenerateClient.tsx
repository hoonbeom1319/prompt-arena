...
// 쓰기는 /api/generate · /api/submit 경유 — supabase 직접 호출 없음 (ARCHITECTURE §2).
export default function GenerateClient({
  ...
  const [activeGenId, setActiveGenId] = useState<string | null>(initialGenerations[0]?.id ?? null)
  const [submittedGenId, setSubmittedGenId] = useState<string | null>(initialSubmittedGenId)
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch('/api/submit', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ challengeId, generationId: selectedGenId }),
    })
