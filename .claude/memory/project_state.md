...
#### 1순위 — 레이아웃 셸 (모든 페이지에 영향)
- **AppBar 컴포넌트** (`components/AppBar.tsx`): 52px 고정, 흰 배경, border-bottom
  - 좌: 뒤로가기 버튼 (32px, rounded-md, back arrow SVG) — 서브 페이지만
  - 중앙: 타이틀 (16px/700)
  - 우: 상태 칩 (badge variant=accent/outline 등)
- **TabBar 컴포넌트** (`components/TabBar.tsx`): 바텀 탭, 흰 배경, border-top
  - 아이콘(21px SVG) + 라벨(10.5px), 활성=sky accent
  - 탭: 아레나(홈 아이콘) / 지난 챌린지(시계 아이콘) / 내 프로필(사람 아이콘)
- **현재 Header 컴포넌트** → AppBar로 교체 (홈: 타이틀만 + 상태칩, 서브: 뒤로가기 + 타이틀)
- layout.tsx에서 body 구조 조정
